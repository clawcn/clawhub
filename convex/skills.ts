  ...
  handler: async (ctx, args) => {
    ...
    const snapshot = buildModerationSnapshot({
      ...
      vtAnalysis:
        reasonMatch?.[1] === 'vt'
          ? {
              status: vtStatus ?? version?.vtAnalysis?.status ?? 'pending',
              verdict: version?.vtAnalysis?.verdict,
              analysis: version?.vtAnalysis?.analysis,
              source: version?.vtAnalysis?.source,
              checkedAt: version?.vtAnalysis?.checkedAt ?? now,
            }
          : version?.vtAnalysis,
      llmAnalysis:
        reasonMatch?.[1] === 'llm'
          ? {
              status: llmStatus ?? version?.llmAnalysis?.status ?? 'pending',
              verdict: version?.llmAnalysis?.verdict,
              confidence: version?.llmAnalysis?.confidence,
              summary: version?.llmAnalysis?.summary,
              dimensions: version?.llmAnalysis?.dimensions,
              guidance: version?.llmAnalysis?.guidance,
              findings: version?.llmAnalysis?.findings,
              model: version?.llmAnalysis?.model,
              checkedAt: version?.llmAnalysis?.checkedAt ?? now,
            }
          : version?.llmAnalysis,
  ...
  handler: async (ctx, args) => {
    ...
    const snapshot = buildModerationSnapshot({
      ...
      vtAnalysis: {
        ...
        analysis: version.vtAnalysis?.analysis,
        source: version.vtAnalysis?.source,
        checkedAt: version.vtAnalysis?.checkedAt ?? Date.now(),
      },
